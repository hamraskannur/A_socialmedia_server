...
export const verify = async (req: Request, res: Response,next: NextFunction) => {  
  ...
  try {
    ...
    if (user == null) return res.status(200).send({ message,status });
    ...
    if (Token == null && user != null) return res.status(200).send({ message,status });

    message = "Invalid link";
    if (Token == null) return res.status(200).send({ message,status });
...
export const getFriendsAccount = async (req: Request, res: Response,next: NextFunction) => {
  try {
    ...
    const friendsAccount = await UserCollection.findOne({ _id: userId }).select('-password');
...
export const updateUserData = async (req: Request, res: Response,next: NextFunction) => {
  try {
    ...
    if (!user || user?._id.equals(new mongoose.Types.ObjectId(userId))) {
